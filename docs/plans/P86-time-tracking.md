...
- **Timer entry tidy rules (post-review, from live testing):** on close (stop/auto-stop), the STORED entry is now discarded if under 2.5 min (accidental tap) and otherwise has both edges snapped to the nearest 5-min grid. Rounding both edges on-close (not the duration, never a running timer) keeps the calendar smooth AND consecutive entries contiguous (a shared transition instant rounds identically on both sides), sidestepping the future-start / crossing problems of snapping stored times. We round the stored data, not the display (no stored-vs-shown lie). Deferred to backlog: the min-height push-down layout (dense short clusters) and a "continue from last stop" start option.
