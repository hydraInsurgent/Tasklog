...
|---|---|---|---|---|
| **v2.13.0** | Task comments + completion log foundation | minor | yes | - |
| **v2.14.0** | Recurrence core (subset + spawn-on-complete + series) | minor | yes | v2.12.0 (time), v2.13.0 (log) |
| **v2.15.0** | Advanced recurrence grammar (3rd-Thu, end conditions) | minor | maybe | v2.14.0 |
| **v2.16.0** | Natural-language recurrence ("every weekday") | minor | no | v2.15.0 |
| **v2.17.0** | Habit tracking (streaks, skip detection) | minor | maybe | v2.14.0 + v2.13.0 |

Five minors (each a real capability + most carry a migration). UX polish may add `.x` patches within a version rather than separate entries. Order is mostly fixed by the dependency column.
...
- **Advanced grammar (v2.15.0):** extend the rule + expander to RRULE's harder bits - `BYSETPOS` ("3rd Thursday"), `BYMONTHDAY`, and end conditions (`UNTIL` / `COUNT`).
- **Natural language (v2.16.0):** the Todoist "type it in words" magic - parse "every weekday", "every 3rd thursday", "every other monday" into the rule. Highest-value for the MCP (Claude speaks language) + a UI quick-entry.
- **Habit tracking (v2.17.0):** streaks and skip detection computed from a series' completed occurrences vs its schedule ("you did 5 of the last 7"), "did you do it?" prompts for missed days, and auto-logging each completion as a comment. Sits on top of everything above.
...
### v2.15.0 - Advanced recurrence grammar `[minor]`
- Extend the rule + expander: `BYSETPOS` ("3rd Thursday"), `BYMONTHDAY`, `UNTIL`/`COUNT` end conditions. UI for these.

### v2.16.0 - Natural-language recurrence `[minor]`
...
### v2.17.0 - Habit tracking `[minor, maybe migration]`
...
- **Five versions or fold some together?** e.g. advanced grammar (v2.15.0) could merge into core (v2.14.0) if we want fewer, bigger releases. Recommend keeping them split - each is independently testable and the core ships sooner.
...
- **Natural-language parsing location:** a dependency/library vs a hand-rolled parser for the supported subset. Decide at v2.16.0 (lean toward a small hand-rolled parser for the agreed phrases to avoid a heavy dep, per the project's "avoid unnecessary frameworks" rule).
