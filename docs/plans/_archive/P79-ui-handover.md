...
- **Chosen: A "One Note"**, with:
  ...
  - **Mood Arc widget** (user's design): the rail mood widget rests as a small heartbeat-style chart of the day's check-ins - x = time of day, y/color = Map of Consciousness level on a red (low) to green (high) range, with a subtle "courage - 200" reference line. A log button opens the check-in flow; on save the chart gains the new point. Clicking the chart may expand it larger (optional).
  - **Feelings wheel picker** (user's design, locked): logging a check-in opens a real concentric feelings wheel (core emotions inner ring, nuanced feelings outer ring - Roberts/Willcox style layout). The user taps ring segments and can **multi-select** feelings; a free-text field still allows own words (the user's own labels always remain first-class). The **MoC score is derived from the selected feelings, never self-tagged** - each feeling in the wheel dataset carries a Hawkins level (e.g. humiliated 20, guilty 30, anxious 100, proud 175, powerful 200, optimistic 310, accepted 350, playful 540, peaceful 600); multi-select averages them. The wheel dataset (all rings + per-feeling MoC values) is data, curated in the implementation plan - the prototype ships core + secondary rings as a first pass. Save closes the wheel and logs the check-in.
  - **Projects Today configuration**: a combobox picks from the user's real Tasklog projects; each picked project gets an inline free-text "what's the focus today" field typed next to its name. Removable per row.
