...
- **Synthetic projected rows in a list response (discriminated union by flag)** - `GET /api/tasks?includeSubtasks=true` returns real `TaskModel` rows PLUS synthetic task-shaped rows built from a task's dated, incomplete subtasks, each flagged `IsSubtask` with `ParentTaskId`/`ParentTitle` and inheriting the parent's project/labels. This lets a subtask "appear as its own card" and pass the existing client-side filters with zero new filter code, while the frontend keys the mixed list by a kind-prefixed `rowKey` (`s-<id>` / `t-<id>`) because a subtask id can numerically collide with a task id. The projection is built from the subtasks ALREADY loaded on the parents that survived the query (not a second DB query), so it inherently respects every filter + `limit`. Reach for a projected-row-with-discriminator when one list needs to show two entity kinds that share a card; always guard id-collision at the React-key and any id-keyed state.
