...
- A task can be tracked as a daily habit (v2.16.0) - tick "Track as a daily habit" on the add/edit forms (or set `isHabit` via Claude). Habits appear on a dedicated Habits view, each showing a current streak (consecutive days checked in, counting back from today with a one-day grace), a last-7-days dot row, and a "Mark done today" toggle. Checking in is one tap and idempotent (one check-in per habit per day); Claude can check a habit in with `log_habit_checkin`. A habit is an ongoing daily check-in, decoupled from recurring tasks (which are finish-once-and-respawn). A calendar heatmap is a deliberate later addition. This completes the recurring + habits program.
