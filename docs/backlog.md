...
- "Continue from last stop time" when starting a timer (Toggl option) - optionally set a new entry's start to the previous entry's (snapped) end for gap-free chaining. Needs a per-start toggle / preference, so belongs with the Settings/preferences item above.
