...
export default function TrackingBar() {
  ...
  async function submit() {
    if (!draft.trim()) {
      setInputError("Enter a task name.");
      return;
    }
    setInputError("");
    const title = draft;
    setDraft("");
    try {
      await quickStart(title);
    } catch {
      setDraft(title); // restore so user can retry
      setInputError("Failed to start. Try again.");
    }
  }
  ...
  return (
    <div className={shellClass}>
      <form
        ...
        className="flex flex-col bg-surface border border-border shadow-lg sm:rounded-2xl sm:w-80"
      >
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <input
            type="text"
            value={draft}
            onChange={(e) => { setDraft(e.target.value); if (inputError) setInputError(""); }}
            placeholder="What are you working on?"
            aria-label="What are you working on?"
            aria-describedby={inputError ? "bar-error" : undefined}
            disabled={pending}
        ...
        {inputError && (
          <p id="bar-error" role="alert" className="px-4 pb-2 text-xs text-danger">
            {inputError}
          </p>
        )}
      </form>
